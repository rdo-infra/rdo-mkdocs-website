...
Name the host with a fully qualified domain name rather than a short-form name to avoid DNS issues with Packstack. FQDN can be defined in ```/etc/hosts``` file with following syntax:
```
<IP adress> <hostname.domain> <hostname>
```
example:
```
10.0.0.2 packstack-host.com packstack-host
```
